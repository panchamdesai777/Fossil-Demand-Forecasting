...
@app.route("/")
def index():
    # Ensure your HTML file is named `index.html` and is placed in a `templates` folder.
...
def fossil_demand_forecasting():
    ...
    try:
        ...
        path = "C:\\Users\\Lenovo\\MSDA\\fossil-stock-forecasting-challenge (1)\\Fossil-Files-Modified\\Demand-Forecasting-Deployment\\Fossil-Demand-Forecasting\\Data\\Feature-Engineering-Test.csv"
        ...
        filtered_df = feature_engineering.apply_saved_arima_models(
            filtered_df, arima_path, "sellout", 3)

        # Generating Final Predictions
        model_pkl_file = "stacking.pkl"
        light_gbm = ModelPredictions()
        final_df = light_gbm.load_lightgbm_model_and_predict(
            model_pkl_file, filtered_df)
        print(final_df)
        response = {"sku_name": final_df["sku_name"].iloc[0], "predictions": round(
            final_df["predictions"].iloc[0], 0)}
